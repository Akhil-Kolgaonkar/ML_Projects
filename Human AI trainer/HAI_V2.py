...
def add_to_faq(new_question, new_answer):
    ...
    existing_questions = [q.lower() for q in df['Question']]
    matching_indices = [i for i, q in enumerate(existing_questions) if q == new_question.lower()]

    if matching_indices:
        for idx in matching_indices:
            df.loc[idx, 'Answer'] = new_answer
        df.to_csv(faq_path, index=False, sep='\t')
        print("Answer updated for", len(matching_indices), "occurrences of the question:", new_question)
    else:
